...
Pipeline:
  ...
  6. Telegram text alert (optional, fail-soft)
...
def step_resolve_miner_daily(cfg: Config) -> Path:
    ...
    emit(f"[STEP 1/5] miner-daily ✓ ({lines} lines)")
...
def step_extract_trend(cfg: Config, work: Path) -> Path:
    ...
    err = work / "trend.err"
    emit("[STEP 2/5] extracting signal-trend...")
    ...
    with out.open() as f:
        lines = sum(1 for _ in f)
    emit(f"[STEP 2/5] signal-trend ✓ ({lines} lines)")
...
def step_synthesise(cfg: Config, work: Path, miner: Path, trend: Path) -> Path:
    ...
    emit(f"[STEP 3/5] synthesising via {cfg.model} (budget ${cfg.budget:.2f})...")
    ...
    with Heartbeat("synthesis"), synth_log.open("wb") as lf, synth_stderr.open("wb") as ef:
        subprocess.run(cmd, stdout=lf, stderr=ef, check=False)
    ...
    with recipe.open() as f:
        lines = sum(1 for _ in f)
    emit(f"[STEP 3/5] synthesis ✓ ({lines} lines)")
    for line in err_text.splitlines():
...
def step_stop_slop(cfg: Config, work: Path, recipe: Path) -> int:
    emit(f"[STEP 4/5] stop-slop gate (min {cfg.min_score})...")
...
def step_katib(cfg: Config, work: Path, recipe: Path) -> None:
    ...
    emit(f"[STEP 5/5] PDF rendered ✓ ({_human_bytes(size_bytes)})")
