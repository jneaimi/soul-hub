...
Usage:
    parse_report.py <report.md> [--run-mode daily]
"""
...
def main():
    ...
    parser.add_argument("--run-mode", default="daily", help="Run mode (daily/weekly)")
    ...
    try:
        db.execute("INSERT INTO analysis_runs (run_date, mode) VALUES (?, ?)", (run_date, args.run_mode))
    except sqlite3.IntegrityError:
        db.execute("UPDATE analysis_runs SET created_at = datetime('now') WHERE run_date = ? AND mode = ?", (run_date, args.run_mode))
    db.commit()
    run_id = db.execute("SELECT id FROM analysis_runs WHERE run_date = ? AND mode = ?", (run_date, args.run_mode)).fetchone()["id"]
