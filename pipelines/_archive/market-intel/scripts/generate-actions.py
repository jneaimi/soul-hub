...
Usage:
    generate-actions.py                   Daily actions (after pipeline)
    generate-actions.py --weekly          Weekly review (after weekly pipeline)
    generate-actions.py --date 2026-03-27 Override date
"""
...
from paths import MI_OUTPUT, MI_CONFIG, MI_DATE

BRAIN = os.path.expanduser("~/SecondBrain")
TASKS_DIR = os.path.join(BRAIN, "02-areas/tasks")
REPORTS_DIR = str(MI_OUTPUT)
ACTIONS_DIR = str(MI_OUTPUT / "actions")
DRAFTS_BASE = str(MI_OUTPUT / "drafts")
IDEAS_DIR = str(MI_OUTPUT / "ideas")
...
def read_active_platforms():
    config_path = str(MI_CONFIG / "content-forge-config.md")
...
def main():
    ...
    parser.add_argument("--date", default=datetime.now().strftime("%Y-%m-%d"), help="Date (default: today)")
    ...
    if args.weekly:
        content = generate_weekly_review(args.date)
        action_file = os.path.join(ACTIONS_DIR, f"{args.date}-weekly-review.md")
        with open(action_file, "w") as f:
            f.write(content)
        print(json.dumps({"type": "weekly", "file": action_file}))

        if not args.no_inject:
            injected = inject_into_task_board(args.date, is_weekly=True)
            if injected:
                print(json.dumps({"injected": True, "target": "task-board"}))
    else:
        content = generate_daily_actions(args.date)
        action_file = os.path.join(ACTIONS_DIR, f"{args.date}-market-intel.md")
        with open(action_file, "w") as f:
            f.write(content)
        print(json.dumps({"type": "daily", "file": action_file}))

        if not args.no_inject:
            injected = inject_into_task_board(args.date, is_weekly=False)
            if injected:
                print(json.dumps({"injected": True, "target": "task-board"}))
