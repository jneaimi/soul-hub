 ...
 *
 *  Per ADR-037 Phase 2. Wraps `engine.updateNote` with a small policy:
 *    - accept  → status: 'accepted',  accepted_on: <today>
 *    - reject  → status: 'rejected',  rejected_on: <today>, reason required
 *    - park    → status: 'parked',    parked_on: <today>, review_after optional
 *
 *  Body: { path: string, action: 'accept' | 'reject' | 'park', reason?: string,
 *          reviewAfter?: string (YYYY-MM-DD) }
 *
...
export const POST: RequestHandler = async ({ request }) => {
	...
	if (currentStatus !== 'proposed') {
		return json(
			{ success: false, error: `Decision is not proposed (current status: ${currentStatus})` },
			{ status: 409 },
		);
	}
