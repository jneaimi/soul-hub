<script lang="ts">
	...
	interface Props {
		...
		/** Emit a `{ heartbeat: { …patch } }` envelope so the parent's
		 *  WhatsApp deep-merge handler updates `channels.whatsapp.heartbeat`. */
	...
	{#if config?.enabled}
		<div class="border-t border-hub-border px-3 pb-3 pt-2.5 space-y-3">
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
				<div>
					<label for="hb-every" class="block text-[11px] text-hub-muted mb-1">
						Cadence (cron expr or duration)
					</label>
					<input
						id="hb-every"
						type="text"
						value={config?.every ?? DEFAULTS.every}
						placeholder="30m"
						oninput={(e) => patchHeartbeat({ every: (e.currentTarget as HTMLInputElement).value })}
				...
				<div>
					...
					<input
						...
						oninput={(e) => patchHeartbeat({ target: (e.currentTarget as HTMLInputElement).value || undefined })}
						class="w-full px-2 py-1.5 bg-hub-bg border border-hub-border rounded text-xs text-hub-text font-mono"
					/>
				</div>
			</div>
			...
			<details bind:open={showAdvanced} class="text-[11px]">
				...
				<div class="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-3">
					...
					<div class="flex items-end">
						<label class="flex items-center gap-2 text-[11px] text-hub-muted">
							<input
								type="checkbox"
								checked={config?.isolatedSession ?? DEFAULTS.isolatedSession}
								onchange={(e) => patchHeartbeat({ isolatedSession: (e.currentTarget as HTMLInputElement).checked })}
							/>
							Isolated session (no chat history)
						</label>
					</div>
				</div>
			</details>
