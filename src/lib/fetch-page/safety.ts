...
/** Domains we always own/route to. Resolved at module load from env or
 *  hardcoded fallback. Operator can extend via SOUL_HUB_INTERNAL_DOMAINS
 *  (comma-separated) when the production surface grows. */
const INTERNAL_DOMAIN_SUFFIXES: readonly string[] = (
	process.env.SOUL_HUB_INTERNAL_DOMAINS?.split(',').map((s) => s.trim().toLowerCase()).filter(Boolean) ??
	['jneaimi.com', 'soul-hub.jneaimi.com']
);
